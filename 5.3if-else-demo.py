...
egitim = input('eğitim (lise ya da üniversite bilgisi): ')
...
tarih = input('aracınız hangi tarihte trafiğe çıktı? (örnek format: 2019/8/9): ')
tarih = tarih.split('/') #/'lardan ayırmak için bu metodu kullandık. ["2019", "8", "9"] olarak gelecek

#line 63'deki kod için açıklama: datetime.datetime() diye yazarsan parametreleri söylüyor.
trafigeCikis = datetime.datetime(int(tarih[0]),int(tarih[1]),int(tarih[2]))
simdi = datetime.datetime.now()
fark = simdi - trafigeCikis
days = fark.days #fark.days komutu fark değişkenini sadece gün üzerinden veriyor.
